...
with st.sidebar:
    ...
    if account_kind == "Portfolio company" and parent_portfolio:
        ...
        with st.expander("Company source setup", expanded=False):
            st.caption(
                "Automatic mode first tries the PE portfolio record, then a company-name .com domain "
                "(for example Seismic → seismic.com; Fleet Data Centers → fleetdatacenters.com), "
                "then public web discovery. You can override either the company homepage or the exact news/insights page."
            )
            current_override = st.session_state.website_overrides.get(prospective_key, "")
            override = st.text_input(
                "Company website URL (optional)",
                value=current_override,
                placeholder="https://www.company.com",
                help="Give the official company homepage if automatic discovery is wrong or uncertain. The app will then discover Newsroom, Insights, Resources, Blog, Press and similar pages inside this domain.",
                key=f"website_override_input_{prospective_key}",
            )
            current_news_override = st.session_state.news_page_overrides.get(prospective_key, "")
            news_override = st.text_input(
                "News / insights / resources page URL (optional)",
                value=current_news_override,
                placeholder="https://www.company.com/newsroom or /resources",
                help="You can paste the exact first-party content page directly. If the company homepage is blank, the app derives the official domain from this URL and uses this page first.",
                key=f"news_override_input_{prospective_key}",
            )
            if override != current_override:
                if override.strip():
                    st.session_state.website_overrides[prospective_key] = override.strip()
                else:
                    st.session_state.website_overrides.pop(prospective_key, None)
            if news_override != current_news_override:
                if news_override.strip():
                    st.session_state.news_page_overrides[prospective_key] = news_override.strip()
                else:
                    st.session_state.news_page_overrides.pop(prospective_key, None)
            st.caption("Tip: you only need one field. A direct newsroom/resources URL is enough to establish the domain.")
...
with st.spinner(f"Refreshing {firm['name']} intelligence…"):
    if firm.get("entity_kind") == "PE firm":
        ...
        official_news, official_news_pages = discover_company_official_news(
            firm.get("official_website", ""),
            firm["name"],
            tuple(firm.get("preferred_news_urls", [])),
        )
...
if firm.get("entity_kind") == "Portfolio company":
    ...
    if firm.get("official_website"):
        confidence = firm.get("website_confidence", "Medium")
        st.caption(
            f"Official website resolved via {firm.get('website_source','public discovery')}: "
            f"{hostname(firm.get('official_website'))} · confidence {confidence}"
        )
        if firm.get("manual_news_page"):
            st.caption(f"Preferred first-party content page · {firm.get('manual_news_page')}")
        if firm.get("source_warning"):
            st.warning(firm.get("source_warning"))
        if confidence in {"Low", "Medium"} and not website_override and not firm.get("manual_news_page"):
            st.info("Automatic website matching is not fully certain. If this is the wrong company, open Company source setup in the sidebar and paste either the official company URL or the exact newsroom/insights/resources URL.")
        ...
        if official_news_count == 0:
            st.warning("No first-party newsroom/blog/resource items were extracted from the confirmed company domain. Open Company source setup in the sidebar and paste either the company homepage or the exact Newsroom / Insights / Resources page.")
    else:
        st.warning("The official website could not be resolved. Open Company source setup in the sidebar and paste either the official company URL or the exact Newsroom / Insights / Resources page. A direct content-page URL is enough for the app to derive the company domain.")
    ...
    if firm.get("entity_kind") == "Portfolio company":
        ...
        if not portfolio:
            ...
            if company_choices:
                ...
                seed = portfolio_row_website(chosen_row, parent_firm)
                website, website_source = discover_official_website(search_name, seed, hostname(parent_firm.get("website")))
                ...
                preview_official_news, _ = discover_company_official_news(website, search_name) if website else ([], [])
...
# =============================================================================
    ...
    section_title("Latest news & growth intelligence", "For portfolio companies, the app searches the confirmed company domain for Newsroom, News, Press, Insights, Resources, Blog, Research and similar first-party sections before adding identity-validated external news.", "Newsroom")
    ...
    if firm.get("entity_kind") == "Portfolio company":
        ...
        if firm.get("manual_news_page"):
            st.success(f"Using supplied first-party content page first: {firm.get('manual_news_page')}")
        if official_news_pages:
            with st.expander("Official content pages discovered", expanded=False):
                for u in official_news_pages:
                    st.write(u)
        else:
            st.info("No first-party content section was confirmed automatically. Use Company source setup in the sidebar to provide the company homepage or the exact Newsroom / Insights / Resources page.")
