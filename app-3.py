...
with st.sidebar:
    ...
    if account_kind == "Portfolio company" and parent_portfolio:
        ...
        with st.expander("Website resolution", expanded=False):
            current_override = st.session_state.website_overrides.get(prospective_key, "")
            override = st.text_input(
                "Official website override",
                value=current_override,
                placeholder="https://www.company.com",
                help="Use this when automatic website discovery resolves an ambiguous company name incorrectly.",
...
if account_kind == "Portfolio company" and selected_portco_row:
    ...
    firm = build_portco_account(parent_firm_key, parent_firm, selected_portco_row, website_override)
else:
...
with st.spinner(f"Refreshing {firm['name']} intelligence…"):
    if firm.get("entity_kind") == "PE firm":
        ...
        google_news = get_google_news(
            f'"{clean_company_search_name(firm["name"])}" (AI OR technology OR digital OR cloud OR data OR cybersecurity OR hiring OR acquisition OR expansion OR transformation)',
            limit=30,
        )
        official_news, official_news_pages = discover_company_official_news(firm.get("official_website", ""))
        uploaded_insights = st.session_state.uploaded_insights.get(selected_firm) or []
        insights = merge_insights(uploaded_insights, local_insights, google_news, official_news)
...
if firm.get("entity_kind") == "Portfolio company":
    ...
    if firm.get("official_website"):
        st.caption(f"Official website resolved via {firm.get('website_source','public discovery')}: {hostname(firm.get('official_website'))}")
    else:
        st.warning("The official website could not be resolved confidently. News and Wikipedia fallback still work, but leadership/jobs may be incomplete. Use Website resolution in the sidebar to override it.")
    ...
    if firm.get("entity_kind") == "Portfolio company":
        ...
        if not portfolio:
            ...
            if company_choices:
                ...
                portco_news = get_google_news(f'"{search_name}" (technology OR AI OR digital OR hiring OR growth OR acquisition)', limit=10)
