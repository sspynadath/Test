...
st.markdown(
    ...
    /* V4.2 navigation + account experience */
    .account-crumb {
        display:flex; align-items:center; gap:.42rem; flex-wrap:wrap;
        color:#64748b; font-size:.78rem; margin:.05rem 0 .65rem;
    }
    .crumb-home { color:#4f46e5; font-weight:760; }
    .crumb-sep { color:#cbd5e1; }
    .crumb-current { color:#172033; font-weight:800; }
    .source-health {
        display:flex; gap:.4rem; flex-wrap:wrap; align-items:center;
        padding:.55rem .72rem; border:1px solid #e7eaf3;
        background:#fbfcff; border-radius:12px; margin:.45rem 0 .75rem;
        color:#64748b; font-size:.73rem;
    }
    .health-dot { width:7px; height:7px; border-radius:999px; display:inline-block; background:#16a34a; }
    .health-dot.warn { background:#d97706; }
    .mini-account-card {
        border:1px solid rgba(255,255,255,.10);
        background:rgba(255,255,255,.055);
        border-radius:13px; padding:.66rem .72rem; margin:.45rem 0 .65rem;
    }
    .mini-account-eyebrow {
        color:#a5b4fc; font-size:.61rem; font-weight:850;
        text-transform:uppercase; letter-spacing:.085em;
    }
    .mini-account-name { color:#fff; font-weight:800; font-size:.88rem; margin:.12rem 0; }
    .mini-account-meta { color:#aeb9ce; font-size:.67rem; line-height:1.35; }
    .nav-section-label {
        color:#94a3b8; font-size:.61rem; font-weight:850;
        letter-spacing:.11em; text-transform:uppercase; margin:.35rem 0 .15rem;
    }
...
def workspace_label(page_name):
    return WORKSPACE_LABELS.get(page_name, page_name)


def render_account_context(account, parent_firm, page_name):
    kind = account.get("entity_kind", "PE firm")
    parent_name = parent_firm.get("name", "") if parent_firm else ""
    if kind == "Portfolio company":
        crumb = (
            f'<div class="account-crumb">'
            f'<span class="crumb-home">{escape(parent_name)}</span>'
            f'<span class="crumb-sep">›</span>'
            f'<span class="crumb-current">{escape(account.get("name",""))}</span>'
            f'<span class="crumb-sep">›</span>'
            f'<span>{escape(page_name)}</span>'
            f'</div>'
        )
    else:
        crumb = (
...
def display_news_card(article, compact=False):
    ...
    source_raw = article.get("source") or "News"
    credibility_raw = article.get("credibility") or credibility_label(article)
    is_official = credibility_raw == "Official company source" or str(source_raw).startswith("Official ·")

    # First-party company feeds are intentionally TITLE-ONLY. The listing page is
    # used to find the headline/date/link; individual article bodies are not loaded.
    summary_raw = "" if is_official else (article.get("summary") or "")
    summary = escape(summary_raw[:350 if compact else 650])
    source = escape(source_raw)
    published = escape(article.get("published") or "")
    signal = escape(article.get("signal_type") or "Other")
    link = article.get("link") or ""
    credibility = escape(credibility_raw)
    credibility_kind = "green" if credibility == "Official company source" else "blue" if credibility == "Priority publication" else "gray"

    html = f"""
    <div class="news-card">
        <div>{badge(signal, 'purple')} {badge(source, 'gray')} {badge(credibility, credibility_kind)}</div>
        <div class="news-title">{title}</div>
        {f'<div class="news-summary">{summary}</div>' if summary else ''}
        {f'<div class="news-meta">{published}</div>' if published else ''}
    </div>
    """
    st.markdown(html, unsafe_allow_html=True)
    if link:
        st.link_button("Open article ↗" if is_official else "Open source ↗", link, use_container_width=compact)
...
# =============================================================================
# SIDEBAR — V4.2 ACCOUNT NAVIGATOR
# =============================================================================
with st.sidebar:
    st.markdown(
        '<div class="brand-lockup"><span class="brand-mark">◆</span>'
        '<span class="brand-title">PE Intelligence</span>'
        '<div class="brand-sub">Coforge account intelligence workspace</div></div>',
        unsafe_allow_html=True,
    )
    st.caption(f"Build v{APP_VERSION}")
    st.markdown('<div class="nav-section-label">Account navigator</div>', unsafe_allow_html=True)
    ...
    parent_firm_key = st.selectbox(
        ...
        help="Choose the sponsor. Its portfolio powers the company switcher.",
    )
    parent_firm = FIRM_CONFIGS[parent_firm_key]

    with st.expander("Find another sponsor", expanded=False):
        firm_search = st.text_input(
            "Search configured firms",
            placeholder="KKR, Permira, Bridgepoint…",
            key="sponsor_search_v42",
        )
        if firm_search.strip():
            ...
            if sponsor_matches:
                quick_sponsor = st.selectbox(
                    "Matching sponsor",
                    sponsor_matches,
                    format_func=lambda key: FIRM_CONFIGS[key]["name"],
                    key="quick_sponsor_v42",
                )
                if st.button("Open sponsor", key="open_quick_sponsor_v42", use_container_width=True):
                    st.session_state.pe_sponsor_selector = quick_sponsor
                    st.session_state.account_scope = "PE firm"
                    st.session_state.page = "Command Center"
                    st.rerun()
            else:
                st.caption("No configured sponsor matched.")
    ...
    parent_portfolio = (
        st.session_state.uploaded_portfolio.get(parent_firm_key)
        or parent_local_portfolio
        or parent_public_portfolio
    )
    ...
    account_kind = st.radio(
        "View",
        scope_options,
        key="account_scope",
        horizontal=True,
        help="Switch between the sponsor account and one of its portfolio companies.",
    )
    ...
    if account_kind == "Portfolio company" and parent_portfolio:
        ...
        if pending_portco and pending_portco in set(pdf["Company"].astype(str)):
            ...
            for suffix in ["sector", "region", "status", "fund"]:
                st.session_state[f"side_{suffix}_{parent_firm_key}"] = "All"

        portco_search = st.text_input(
            "Portfolio company",
            placeholder="Search company name…",
            key=f"portco_search_{parent_firm_key}",
        )

        with st.expander("Filter portfolio companies", expanded=False):
            cf1, cf2 = st.columns(2)
            with cf1:
                sector_filter = st.selectbox(
                    "Sector", ["All"] + safe_unique(pdf["Sector"]),
                    key=f"side_sector_{parent_firm_key}",
                )
            with cf2:
                region_filter = st.selectbox(
                    "Region", ["All"] + safe_unique(pdf["Region"]),
                    key=f"side_region_{parent_firm_key}",
                )
            cf3, cf4 = st.columns(2)
            with cf3:
                status_filter = st.selectbox(
                    "Status", ["All"] + safe_unique(pdf["Status"]),
                    key=f"side_status_{parent_firm_key}",
                )
            with cf4:
                fund_filter = st.selectbox(
                    "Fund", ["All"] + safe_unique(pdf["Fund"]),
                    key=f"side_fund_{parent_firm_key}",
                )

        sector_filter = st.session_state.get(f"side_sector_{parent_firm_key}", "All")
        region_filter = st.session_state.get(f"side_region_{parent_firm_key}", "All")
        status_filter = st.session_state.get(f"side_status_{parent_firm_key}", "All")
        fund_filter = st.session_state.get(f"side_fund_{parent_firm_key}", "All")

        filtered_portcos = pdf.copy()
        if portco_search:
            mask = filtered_portcos.astype(str).apply(
                lambda c: c.str.contains(portco_search, case=False, na=False)
            ).any(axis=1)
        ...
        if not choices:
            st.warning("No matches. Showing the full portfolio instead.")
            choices = safe_unique(pdf["Company"])

        chosen_portco = st.selectbox(
            "Open company",
            choices,
            key=f"sidebar_portco_{parent_firm_key}",
        ...
        st.markdown(
            f'''<div class="mini-account-card">
                <div class="mini-account-eyebrow">Portfolio company</div>
                <div class="mini-account-name">{escape(chosen_portco)}</div>
                <div class="mini-account-meta">
                    {escape(selected_portco_row.get("Sector") or "Sector not captured")}
                    &nbsp;·&nbsp;
                    {escape(selected_portco_row.get("Region") or "Region not captured")}
                </div>
            </div>''',
            unsafe_allow_html=True,
        )
        ...
        if st.button(
            f"← Back to {parent_firm['name']}",
            key=f"sidebar_back_to_sponsor_{parent_firm_key}",
        ...
        st.markdown(
            f'''<div class="mini-account-card">
                <div class="mini-account-eyebrow">PE sponsor</div>
                <div class="mini-account-name">{escape(parent_firm["name"])}</div>
                <div class="mini-account-meta">{escape(parent_firm.get("category","Private equity"))}</div>
            </div>''',
            unsafe_allow_html=True,
        )
        if parent_portfolio and st.button(
            "Browse portfolio companies →",
            key=f"sidebar_browse_portfolio_{parent_firm_key}",
            use_container_width=True,
        ):
    ...
    pages = [
        "Command Center", "Portfolio", "Leadership", "Hiring & Skills",
        "Technology Signals", "Newsroom", "Opportunity Lab", "AI Analyst", "Data Hub",
    ]
    ...
    with st.expander("Data & source status", expanded=False):
        st.markdown(
            f"{badge('Official sites', 'green')} "
            f"{badge('External news', 'blue')} "
            f"{badge('ATS / careers', 'purple')}",
            unsafe_allow_html=True,
        )
        if requests is None or BeautifulSoup is None:
            st.warning("Install requests + beautifulsoup4 for website discovery.")
        st.caption("Session uploads override public extraction for the active account.")
...
if account_kind == "Portfolio company" and selected_portco_row:
    ...
    # V4.1 source resolution is independent of the legacy requests-only resolver.
...
render_account_context(firm, parent_firm, page)
hero(firm, priority_score, confidence_score, len(insights))

if firm.get("entity_kind") == "Portfolio company":
    qa1, qa2, qa3, qa4 = st.columns([1.25, 1, 1, 1])
    with qa1:
        if st.button(
            f"← {parent_firm['name']}",
            key=f"top_back_sponsor_{selected_firm}",
            use_container_width=True,
        ):
            st.session_state.pending_account_scope = "PE firm"
            st.session_state.page = "Command Center"
            st.rerun()
    with qa2:
        if st.button("Sponsor portfolio", key=f"top_sponsor_portfolio_{selected_firm}", use_container_width=True):
            st.session_state.page = "Portfolio"
            st.rerun()
    with qa3:
        if st.button("News & insights", key=f"top_news_{selected_firm}", use_container_width=True):
            st.session_state.page = "Newsroom"
            st.rerun()
    with qa4:
        if st.button("Opportunities", key=f"top_opp_{selected_firm}", use_container_width=True):
            st.session_state.page = "Opportunity Lab"
            st.rerun()
else:
    ...
    with qa1:
        if parent_portfolio and st.button(
            "Browse portfolio companies",
            key=f"top_browse_portco_{parent_firm_key}",
            use_container_width=True,
        ):
            st.session_state.pending_account_scope = "Portfolio company"
            st.session_state.page = "Command Center"
            st.rerun()
    with qa2:
        if st.button("Leadership", key=f"top_leaders_{parent_firm_key}", use_container_width=True):
            st.session_state.page = "Leadership"
            st.rerun()
    with qa3:
        if st.button("Hiring", key=f"top_hiring_{parent_firm_key}", use_container_width=True):
            st.session_state.page = "Hiring & Skills"
            st.rerun()
    with qa4:
        if st.button("Opportunities", key=f"top_opp_pe_{parent_firm_key}", use_container_width=True):
            st.session_state.page = "Opportunity Lab"
            st.rerun()
...
if firm.get("entity_kind") == "Portfolio company":
    ...
    # V4.2 source status is always visible; manual controls stay one click away.
    resolved_site = firm.get("official_website", "")
    source_ok = bool(resolved_site and official_news_count > 0)
    source_dot = "health-dot" if source_ok else "health-dot warn"
    source_text = (
        f"{official_news_count} first-party article{'s' if official_news_count != 1 else ''}"
        if resolved_site else "official website not confirmed"
    )
    st.markdown(
        f'''<div class="source-health">
            <span class="{source_dot}"></span>
            <b>Company sources</b>
            <span>·</span>
            <span>{escape(hostname(resolved_site) or "unresolved")}</span>
            <span>·</span>
            <span>{escape(source_text)}</span>
            <span>·</span>
            <span>{external_news_count} validated external item{'s' if external_news_count != 1 else ''}</span>
        </div>''',
        unsafe_allow_html=True,
    )

    source_attention = not resolved_site or official_news_count == 0
    expander_title = "⚙ Fix / confirm company sources" if source_attention else "⚙ Company source settings"

    with st.expander(expander_title, expanded=source_attention):
        st.caption(
            "Automatic discovery is the default. Override it only when the company domain "
            "or its Newsroom / Insights / Resources page is wrong."
        )
        with st.form(f"source_controls_{selected_firm}", clear_on_submit=False, enter_to_submit=True):
            ...
            a1, a2 = st.columns(2)
            with a1:
                apply_sources = st.form_submit_button("Apply & refresh", type="primary", use_container_width=True)
            with a2:
                reset_sources = st.form_submit_button("Use automatic discovery", use_container_width=True)
        ...
        if apply_sources:
            ...
            clear_news_cache()
            st.session_state.source_refresh_notice[selected_firm] = "Sources updated and refreshed."
            st.rerun()
        ...
        if reset_sources:
            ...
            st.session_state.news_page_overrides.pop(selected_firm, None)
            clear_news_cache()
            st.session_state.source_refresh_notice[selected_firm] = "Manual sources cleared. Automatic discovery refreshed."
            st.rerun()
        ...
        if official_news_count == 0 and official_news_status.get("errors"):
            with st.expander("Technical scrape details", expanded=False):
...
# =============================================================================
    ...
    if firm.get("entity_kind") == "Portfolio company":
        ...
        section_title("Sponsor portfolio", "Search the sponsor portfolio and open any company as a complete intelligence account.", "Portfolio")
        ...
        if not portfolio:
            ...
            st.divider()
            section_title(
                "Open a portfolio company",
                "Select a company and move directly into its full intelligence workspace. "
                "Website, news, people and hiring load only after you open it, keeping this page fast.",
                "Research",
            )
            ...
            if company_choices:
                chosen = st.selectbox(
                    "Portfolio company",
                    company_choices,
                    key=f"portfolio_launcher_company_{parent_firm_key}",
                )
                chosen_row = filtered[filtered["Company"] == chosen].iloc[0].to_dict()

                pc1, pc2, pc3, pc4 = st.columns(4)
                pc1.metric("Sector", chosen_row.get("Sector") or "—")
                pc2.metric("Region", chosen_row.get("Region") or "—")
                pc3.metric("Status", chosen_row.get("Status") or "—")
                pc4.metric("Fund / strategy", chosen_row.get("Fund") or "—")

                open_col, hint_col = st.columns([1, 2.2])
                with open_col:
                    if st.button(
                        f"Open {chosen} →",
                        type="primary",
                        use_container_width=True,
                        key=f"open_portco_full_{parent_firm_key}",
                    ):
                ...
                with hint_col:
                    st.caption(
                        "Opening the company switches Command Center, Leadership, Hiring, "
                        "Technology, News, Opportunities and AI Analyst to that company."
                    )
...
# =============================================================================
    ...
    section_title("News & insights", "First-party company content is prioritised, with identity-validated external reporting added separately.", "Intelligence")
