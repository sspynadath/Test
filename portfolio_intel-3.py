...
# -----------------------------------------------------------------------------
# Some first-party newsrooms (Seismic is a good example) return only a
# "Loading component..." shell to requests/BeautifulSoup because their article
# cards are populated in the browser. Jina Reader renders pages with a browser
# and exposes the result as text/Markdown. Basic Reader usage works without an
# API key; an optional JINA_API_KEY can be supplied through Streamlit secrets or
# an environment variable for a higher rate limit.
JINA_READER_BASE = "https://r.jina.ai/"


def _optional_jina_key():
    try:
        value = st.secrets.get("JINA_API_KEY", "")
        if value:
            return str(value).strip()
    except Exception:
        pass
    return str(os.environ.get("JINA_API_KEY", "") or "").strip()


@st.cache_data(ttl=21600, show_spinner=False)
def fetch_rendered_reader(url, timeout=18):
    """Return rendered Markdown/text for a public URL using Jina Reader.

    This is a fallback, not the primary transport. A failure simply returns an
    empty string so the rest of the intelligence pipeline keeps working.
    """
    if not requests or not is_http_url(url):
        return ""
    headers = {
        "User-Agent": WEB_HEADERS.get("User-Agent", "Mozilla/5.0"),
        "Accept": "application/json, text/plain;q=0.9, */*;q=0.8",
        "X-Return-Format": "markdown",
    }
    key = _optional_jina_key()
    if key:
        ...
        if "json" in ctype:
            try:
                ...
                if isinstance(payload, dict):
                    ...
                    for key_name in ["content", "markdown", "text"]:
                        value = data.get(key_name) if isinstance(data, dict) else ""
                        if value:
                            return str(value)[:4_000_000]
            except Exception:
                pass
...
_READER_NOISE = {
    "newsroom", "news", "press releases", "press release", "resources", "resource center",
    "blog", "insights", "latest posts", "view more press releases", "read more", "learn more",
    "home", "company", "careers", "contact us", "about seismic", "about us", "get a demo",
}


def _reader_date_near(text, start, end):
    # Search immediately around a link/title. Most newsroom cards put the date
    # on the following line, but some place it immediately before the title.
    region = text[max(0, start - 180): min(len(text), end + 220)]
    for pattern in _READER_DATE_RES:
        match = pattern.search(region)
        if match:
            return compact_text(match.group(0), 100)
    return ""


def _clean_reader_title(value):
    title = compact_text(re.sub(r"[*_#`]+", "", str(value or "")), 260).strip(" -–—|:")
    if not (12 <= len(title) <= 250):
        return ""
    if title.lower() in _READER_NOISE:
        return ""
    if title.lower().startswith(("image:", "download ", "return to ")):
        return ""
    return title


@st.cache_data(ttl=21600, show_spinner=False)
def discover_rendered_page_articles(page_url, website, company_name="", limit=50):
    """Extract first-party article/resource links from a JS-rendered landing page.

    Every linked item is still required to resolve to the confirmed official
    domain, so the rendered fallback cannot introduce third-party homonyms.
    """
    text = fetch_rendered_reader(page_url)
    if not text:
        return []

    rows, seen = [], set()
    for match in _READER_LINK_RE.finditer(text):
        title = _clean_reader_title(match.group(1))
        link = normalize_user_url(match.group(2).replace("&amp;", "&"))
        if not title or not link or link in seen:
            continue
        if not _same_official_domain(link, website) or not _content_url(link, website):
            continue
        # Avoid treating the supplied landing page itself or navigation links as
        # a news item. We want article/resource detail pages here.
        if link.rstrip("/") == page_url.rstrip("/"):
            continue
        low_path = urlparse(link).path.lower().rstrip("/")
        if low_path.endswith(("/newsroom", "/news", "/press", "/press-releases", "/blog", "/resources", "/insights")):
            continue
        seen.add(link)
        published = _reader_date_near(text, match.start(), match.end())
        # Take a short nearby excerpt as evidence/summary when available.
        nearby = compact_text(text[match.end(): match.end() + 700], 650)
        nearby = re.sub(r"https?://\S+", "", nearby).strip()
        rows.append({
            "title": title,
            "summary": nearby,
            "link": link,
            ...
            "signal_type": classify_news_signal(f"{title} {nearby}"),
            "credibility": "Official company source",
            "retrieval_method": "Rendered first-party page",
        })
        if len(rows) >= limit:
            break

    # Some rendered readers return the visible card text but omit links. Keep a
    # conservative title+date fallback so a manually supplied newsroom can still
    # populate the app instead of falling all the way back to unrelated external
    # news. These records link to the supplied first-party landing page.
    if len(rows) < 3:
        lines = [compact_text(re.sub(r"[*_#`]+", "", ln), 320) for ln in text.splitlines()]
        lines = [ln.strip() for ln in lines if ln.strip()]
        for idx, line in enumerate(lines):
            title = _clean_reader_title(line)
            if not title:
                continue
            if company_name and len(company_name_tokens(company_name)) <= 1 and company_name.lower() not in title.lower():
                # For ambiguous one-word brands such as Seismic, an unlinked
                # rendered-card title must itself identify the company.
                continue
            ...
            for pattern in _READER_DATE_RES:
                ...
                if m:
                    published = compact_text(m.group(0), 100)
                    break
            if not published:
                continue
            key = (title.lower(), published.lower())
            if key in seen:
                continue
            seen.add(key)
            rows.append({
                "title": title,
                "summary": "First-party item discovered from the rendered company content page.",
                "link": page_url,
                "published": published,
                "source": f"Official · {hostname(website)}",
                "signal_type": classify_news_signal(title),
                "credibility": "Official company source",
                "retrieval_method": "Rendered first-party page (card text)",
            })
            if len(rows) >= limit:
                break

    return _sort_news_newest(normalize_insights(rows))[:limit]
...
def discover_company_official_news(website, company_name="", preferred_news_urls=()):
    ...
    if not website:
        return [], []
    working, news = [], []
    preferred_clean = [normalize_user_url(x) for x in (preferred_news_urls or ()) if normalize_user_url(x)]
    preferred_set = {x.rstrip("/") for x in preferred_clean}
    news_pages = discover_official_source_pages(website, company_name, tuple(preferred_clean))[:24]
    reader_budget = 2  # keep the browser-rendered fallback bounded and cacheable

    # 1) Direct first-party pages, including a user-supplied newsroom/insights URL.
    for url in news_pages:
        html, final = fetch_public_html(url)
        final = final or url
        if final and final not in working:
            working.append(final)
        before_direct = len(news)
        if html:
            try:
        ...
        # If the exact page supplied by the user is JS-rendered (or a discovered
        # content hub yielded almost nothing in raw HTML), render it with the
        # browser-backed Reader fallback and extract the first-party cards there.
        direct_added = len(news) - before_direct
        looks_dynamic = bool(html and any(marker in html.lower() for marker in ["loading component", "loading...", "__next_data__", "data-reactroot"]))
        is_preferred = url.rstrip("/") in preferred_set or final.rstrip("/") in preferred_set
        if reader_budget > 0 and (is_preferred or looks_dynamic or (direct_added < 2 and len(working) <= 2)):
            rendered_rows = discover_rendered_page_articles(final, website, company_name, limit=50)
            if rendered_rows:
                news.extend(rendered_rows)
            reader_budget -= 1

        if not html:
            # Raw HTML can be blocked while the rendered reader still succeeds.
            # Do not discard the page before trying the fallback above.
            continue
